...
/**
 * Converts a free-text location or coordinates into coordinates and location name.
 */
async function resolveLocation(locationInput, apiKey) {
    let locationQuery = null;
    let lat = null;
    let lon = null;

    if (typeof locationInput === 'object' && locationInput !== null) {
        locationQuery = locationInput.location || null;
        lat = locationInput.lat ?? null;
        lon = locationInput.lon ?? null;
    } else {
        locationQuery = locationInput || null;
    }
        ...
        try {
            ...
            if (res.ok) {
                ...
                if (results && results.length) {
                    ...
                    return { lat: parsedLat, lon: parsedLon, name };
                }
            }
        } catch (e) {
            console.warn('Reverse geocoding failed:', e.message);
        }
        return { lat: parsedLat, lon: parsedLon, name: locationQuery || null };
    }
...
/**
 * Fetches current conditions + forecast for a given location or coordinates.
 */
export async function getWeather(locationInput, maybeCoords) {
    let locationQuery = null;
    let lat = null;
    let lon = null;
    ...
    if (typeof locationInput === 'object' && locationInput !== null) {
        ...
        locationQuery = locationInput || null;
        if (maybeCoords && typeof maybeCoords === 'object') {
            lat = maybeCoords.lat ?? null;
            lon = maybeCoords.lon ?? null;
        }
    }
    ...
    if (!apiKey) {
        try {
            ...
            const deployedUrl = `https://dashboard-tau-weld-15.vercel.app/api/weather${queryStr}`;
            const res = await fetch(deployedUrl, { next: { revalidate: 60 } });
            if (res.ok) {
                const liveData = await res.json();
                const fallback = getFallbackWeatherData(liveData.location || locationQuery || 'Local Area');
                return {
                    ...fallback,
    ...
    try {
        const { lat: resolvedLat, lon: resolvedLon, name: resolvedName } = await resolveLocation({ locationQuery, lat, lon }, apiKey);

        const currentUrl = `https://api.openweathermap.org/data/2.5/weather?lat=${resolvedLat}&lon=${resolvedLon}&units=metric&appid=${apiKey}`;
        const forecastUrl = `https://api.openweathermap.org/data/2.5/forecast?lat=${resolvedLat}&lon=${resolvedLon}&units=metric&appid=${apiKey}`;
        ...
        const finalLocationName = resolvedName || (current.name ? [current.name, current.sys?.country].filter(Boolean).join(', ') : null) || DEFAULT_LOCATION_NAME;
        ...
        return {
            ...
            hourly: hourly.length ? hourly : getFallbackWeatherData(finalLocationName).hourly,
            daily: daily.length ? daily : getFallbackWeatherData(finalLocationName).daily,
            live_trend_percent: '+23.8%',
